import Link from "next/link";
import { Fragment } from "react";
import { connect } from "react-redux";

interface MenuItem {
  icon?: React.ReactNode;
  title: string;
  badgetxt?: string;
  class?: string;
  selected?: boolean;
  active?: boolean;
  dirchange?: boolean;
  type?: "link" | "empty" | "sub";
  path?: string;
  children?: MenuItem[];
  menutitle?: boolean;
}

interface MenuLoopProps {
  local_varaiable: unknown; // Adjust if you have a state type
  MenuItems: MenuItem;
  toggleSidemenu: (event: React.MouseEvent<HTMLAnchorElement>, menu: MenuItem) => void;
  level: number;
  HoverToggleInnerMenuFn: (event: React.MouseEvent<HTMLAnchorElement>, menu: MenuItem) => void;
}

const Menuloop: React.FC<MenuLoopProps> = ({
  local_varaiable,
  MenuItems,
  toggleSidemenu,
  level,
  HoverToggleInnerMenuFn,
}) => {
  const handleClick = (event: React.MouseEvent<HTMLAnchorElement>) => {
    event.preventDefault(); // Prevents default navigation
    // Additional logic can go here
  };

  return (
    <Fragment>
      <Link
        href="#!"
        scroll={false}
        className={`side-menu__item ${MenuItems?.selected ? "active" : ""}`}
        onClick={(event) => {
          event.preventDefault();
          toggleSidemenu(event, MenuItems);
        }}
        onMouseEnter={(event) => HoverToggleInnerMenuFn(event, MenuItems)}
      >
        <span
          className={`hs-tooltip inline-block [--placement:right] leading-none ${
            local_varaiable?.dataVerticalStyle === "doublemenu" ? "" : "hidden"
          }`}
        >
          <button
            type="button"
            className="hs-tooltip-toggle  inline-flex justify-center items-center"
          >
            {MenuItems.icon}
            <span
              className="hs-tooltip-content hs-tooltip-shown:opacity-100 hs-tooltip-shown:visible opacity-0 transition-opacity inline-block absolute invisible z-10 py-1 px-2 bg-black text-xs font-medium text-white rounded shadow-sm dark:bg-neutral-700"
              role="tooltip"
            >
              {MenuItems.title}
            </span>
          </button>
        </span>
        {local_varaiable?.dataVerticalStyle !== "doublemenu" ? MenuItems.icon : ""}
        <span className={`${level === 1 ? "side-menu__label" : ""}`}>
          {MenuItems.title}{" "}
          {MenuItems.badgetxt ? (
            <span className={MenuItems.class}>{MenuItems.badgetxt}</span>
          ) : (
            ""
          )}
        </span>
        <i className="fe fe-chevron-right side-menu__angle"></i>
      </Link>
      <ul
        className={`slide-menu child${level} ${
          MenuItems.active ? "double-menu-active" : ""
        } ${MenuItems?.dirchange ? "force-left" : ""}`}
        style={MenuItems.active ? { display: "block" } : { display: "none" }}
      >
        {level <= 1 && (
          <li className="slide side-menu__label1">
            <Link href="#!" scroll={false}>
              {MenuItems.title}
            </Link>
          </li>
        )}
        {MenuItems.children?.map((firstlevel, index) => (
          <li
            className={`${firstlevel?.menutitle ? "slide__category" : ""} ${
              firstlevel?.type === "empty" ? "slide" : ""
            } ${firstlevel?.type === "link" ? "slide" : ""} ${
              firstlevel?.type === "sub" ? "slide has-sub" : ""
            } ${firstlevel?.active ? "open" : ""} ${
              firstlevel?.selected ? "active" : ""
            }`}
            key={index}
          >
            {firstlevel.type === "link" && (
              <Link
                href={firstlevel.path || "#!"}
                className={`side-menu__item ${firstlevel.selected ? "active" : ""}`}
              >
                {firstlevel.icon}
                <span>
                  {firstlevel.title}{" "}
                  {firstlevel.badgetxt ? (
                    <span className={firstlevel.class}>{firstlevel.badgetxt}</span>
                  ) : (
                    ""
                  )}
                </span>
              </Link>
            )}
            {firstlevel.type === "empty" && (
              <Link href="#!" className="side-menu__item" onClick={handleClick}>
                {firstlevel.icon}
                <span>
                  {firstlevel.title}{" "}
                  {firstlevel.badgetxt ? (
                    <span className={firstlevel.class}>{firstlevel.badgetxt}</span>
                  ) : (
                    ""
                  )}
                </span>
              </Link>
            )}
            {firstlevel.type === "sub" && (
              <Menuloop
                MenuItems={firstlevel}
                toggleSidemenu={toggleSidemenu}
                HoverToggleInnerMenuFn={HoverToggleInnerMenuFn}
                level={level + 1} local_varaiable={undefined}              />
            )}
          </li>
        ))}
      </ul>
    </Fragment>
  );
};

const mapStateToProps = (state: unknown) => ({
  local_varaiable: state,
});

export default connect(mapStateToProps)(Menuloop);